...
function addTags(jname, e, obj) {
    ...
    LoadQuickDiv(e, 'archives_do.php?dopost=tagsDlg&qstr=' + qstr + '&dojob=' + jname + '&rnd=' + Math.random(), 'tagsDlg', '450px', '205px');
    ChangeFullDiv('show', screeheight);
}

function addKeywords(jname, e, obj) {
    var qstr = getCheckboxItem();
    var screeheight = document.body.clientHeight + 20;
    if (qstr == '') {
        alert('必须选择一个或多个文档！');
    ...
    LoadQuickDiv(e, 'archives_do.php?dopost=keywordsDlg&qstr=' + qstr + '&dojob=' + jname + '&rnd=' + Math.random(), 'keywordsDlg', '450px', '205px');
    ChangeFullDiv('show', screeheight);
}
